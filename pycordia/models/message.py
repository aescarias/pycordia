...
class Message:
    def __init__(self, data: dict):
        ...
        self.mentions: List[User] = [User(mention) for mention in data.get("mentions", [])]
        self.mention_roles: List[Role] = [Role(role) for role in data.get("mention_roles", [])]
        self.mention_channels: List[ChannelMention] = [ChannelMention(cm) for cm in data.get("mention_channels", [])]
        ...
        self.message_reference: MessageReference = MessageReference(data.get("message_reference", {}), data.get("referenced_message", {}))
        
        self.interaction: Interaction = Interaction(data.get("interaction", {}))
        self.thread = None
        self.components = None
        self.sticker_items: List[StickerItem] = [StickerItem(stick) for stick in data.get("sticker_items", [])] 

    @classmethod
    def create(cls, *, content: str = "", reply_to: MessageReference = None, 
        embeds: List[embed.Embed] = None, attachments: List[Attachment] = None
    ):
        if reply_to:
    ...
    async def delete(self, client):
        async with aiohttp.ClientSession() as session:
            ...
            await session.delete(url, headers={"Authorization": f"Bot {client.ws.bot_token}"})
